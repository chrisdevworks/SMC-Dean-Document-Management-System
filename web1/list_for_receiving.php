...
<div class="col-lg-12">
    <a role="button" href="./index.php?page=new_document&trackingnumber=<?php echo date("Y") ?>-<?php echo date("md") ?>-<?php echo $numrand ?>-<?php echo $numrand2 ?>" class="btn btn-info mr-3 my-3 shadow-sm btn-sm">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered table-hover w-auto small" id="list">
                    ...
                    <tbody>
                        ...
                        //SELECT * FROM documents WHERE current_office_id = '{$userOfficeID}' AND recipient_office_id IS NULL
                        $qry = connectdb()->query("SELECT * FROM documents WHERE recipient_office_id = '{$userOfficeID}' AND doc_status = 'ongoing' AND time_release IN (SELECT max(time_release) FROM documents GROUP BY tracking_number) ");
                        while ($row = $qry->fetch_assoc()) :
                        ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <!-- <button type="button" class="btn btn-success btn-sm receive_docu" data-id="<?php echo $row['tracking_number'] ?>" data-appnt="Recieve" >
                                            RECEIVE
                                        </button> -->
                                        <button data-origin="<?php $row['originating_office_id'] ?>" data-id="<?php $row['document_id'] ?>" data-track="<?php echo $row['tracking_number'] ?>" class="receive receive_docu text-white btn btn-outline-secondary bg-gradient-info shadow-sm btn-sm" href="#">RECEIVE</button>
                                    </div>
                                </td>
                            </tr>
                        <?php

        ...
        <div class="card-body">
            <form action="" id="manage_document" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="exampleFormControlTextarea1" name="remarks" rows="4" maxlength="250" required></textarea>
                ...
                <div class="col-lg-12 text-right justify-content-center d-flex">
                    <button class="btn btn-outline-secondary bg-gradient-gray mr-2" type="submit">Tag as Endpoint</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $('#manage_document').submit(function(e) {
        ...
        _confrelease("Are you sure you want to tag this document as endpoint? Doing so indicates that your office is the END of its paper trail.", data = new FormData($(this)[0]), function(confirmed) {
            if (confirmed) {
                $.ajax({
                    url: 'ajax.php?action=endpoint_docs',
                    data: data,
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    success: function(resp) {
                        if (resp == 2) {
                            message_toast("<p>Your office doesn't have access to this document or haven't received it yet </p>", "warning");
                            setTimeout(function() {
                                location.reload()
                            }, 1200)
                        } 
                        else if (resp == 4) {
                            message_toast("Already released.", "error");
                            setTimeout(function() {
                                location.reload()
                            }, 750)
                        } else if (resp == 3) {
                            message_toast("This tracking number is already in the endpoint.", "error");
                            setTimeout(function() {
                                location.reload()
                            }, 750)
                        }
